...
//         i1++;
//       } else {
//         while (i1 < a1->size_sparse() && a2->indices()[i2] >
//         a1->indices()[i1])
//           i1++;
//       }
//     }
...
// template<typename T>
// template<typename K>
// K BaseArray<T>::dot(const BaseArray<T> &array) const {
//     if (_size != array.size()) TICK_ERROR("Arrays don't have the same size");
...
//     // Case dense/dense
//     if (is_dense() && array.is_dense()) {
//         return (tick::vector_operations<T>{}).dot(this->size(), this->data(),
//         array.data());
//     }
...
//         while (true) {
//             if (i1 >= a1->size_sparse()) break;
//             while (i2 < a2->size_sparse() && a2->indices()[i2] <
//             a1->indices()[i1]) i2++; if (i2 >= a2->size_sparse()) break; if
