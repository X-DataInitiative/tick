 ...
 * class `SSparseArrayPtr` and never access this class directly. Thus the only
 * constructor to be used is the `SSparseArray<T>::new_ptr()` constructor. In
...
class SSparseArray : public SparseArray<T> {
 ...
 public:
  ...
  //! @brief Sets the data/indices allocation owner with either a numpy array or
  //! nullptr \param data A pointer to the data array \param indices A pointer
  //! to the indices array \param size The size of the full array \param
  //! size_sparse The size of non zero values \param data_owner A pointer to the
  //! numpy array (if nullptr, then the data allocation is owned by the object
  //! itself) that owns _data allocation \param indices_owner A pointer to the
  //! numpy array (if nullptr, then the data allocation is owned by the object
  //! itself) that owns _indices allocation
  virtual void set_data_indices(T *data, INDICE_TYPE *indices, ulong size,
                                ulong size_sparse, void *owner_data = nullptr,
 ...
 public:
  ...
  //     SSparseArray<T>::new_ptr(size, size_sparse) which returns a shared
  //     pointer SSparseArray<T>::new_ptr(array) which returns a shared pointer
  //
  ...
  //! \return A shared pointer to the corresponding shared sparsearray of the
  //! given size and size_sparse. So if size_sparse != 0 then allocation is
  //! performed. \warning : Right after this call, you are supposed to fill up
  //! the indices array with valid values
  static std::shared_ptr<SSparseArray<T>> new_ptr(ulong size,
                                                  ulong size_sparse);
 ...
 protected:
   ...
   * nullptr
   */
  virtual void _clear(bool &flag_desallocate_data,
...
#ifdef PYTHON_LINK
template <typename T>
void SSparseArray<T>::set_data_indices(T *data, INDICE_TYPE *indices,
                                       ulong size, ulong size_sparse,
                                       void *data_owner, void *indices_owner) {
  clear();
  _data = data;
  _indices = indices;
  _size = size;
...
#ifdef PYTHON_LINK
template <typename T>
void SSparseArray<T>::give_data_indices_owners(void *data_owner,
                                               void *indices_owner) {
#ifdef DEBUG_SHAREDARRAY
  if (_data_owner == nullptr)
    std::cout << "SSparseArray : SetOwner data_owner=" << data_owner << " on "
              << this << std::endl;
  else
    std::cout << "SSparseArray : ChangeDataOwner data_owner=" << _data_owner
              << " -> " << data_owner << " on " << this << std::endl;

  if (_indices_owner == nullptr)
    std::cout << "SSparseArray : SetOwner indices_owner=" << data_owner
              << " on " << this << std::endl;
  else
    std::cout << "SSparseArray : ChangeIndicesOwner indices_owner="
              << _indices_owner << " -> " << indices_owner << " on " << this
              << std::endl;
#endif
...
#ifdef DEBUG_SHAREDARRAY
  n_allocs++;
  std::cout << "SSparseArray Constructor (->#" << n_allocs
...
// @brief The only constructor to be used
template <typename T>
std::shared_ptr<SSparseArray<T>> SSparseArray<T>::new_ptr(ulong size,
                                                          ulong size_sparse) {
  std::shared_ptr<SSparseArray<T>> aptr =
      std::make_shared<SSparseArray<T>>(size);
...
std::shared_ptr<SSparseArray<T>> SSparseArray<T>::new_ptr(SparseArray<T> &a) {
  std::shared_ptr<SSparseArray<T>> aptr =
      SSparseArray<T>::new_ptr(a.size(), a.size_sparse());
  if (a.size_sparse() != 0) {
...
template <typename T>
void SSparseArray<T>::_clear(bool &flag_desallocate_data,
                             bool &flag_desallocate_indices) {
...
#ifdef DEBUG_SHAREDARRAY
      std::cout << "SSparseArray Clear : " << this
                << " decided not to free indices=" << _indices
                << " since owned by owner=" << _indices_owner << std::endl;
#endif
      _indices_owner = nullptr;
    } else {
#ifdef DEBUG_SHAREDARRAY
      std::cout << "SSparseArray Clear : " << this
                << " decided to free indices=" << _indices << std::endl;
...
#ifdef DEBUG_SHAREDARRAY
  n_allocs--;
  std::cout << "SSparseArray Destructor (->#" << n_allocs
            << ") : ~SSparseArray on " << this << std::endl;
#endif
  clear();
}

// @brief Returns a shared pointer to a SSparseArray encapsulating the sparse
