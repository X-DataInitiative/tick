 ...
 * shared with the Python interpreter then this is ONLY handled by SSparseArray
 * classes through their smart pointers `SSparseArrayPtr`.
 *
...
class SparseArray : public BaseArray<T> {
 ...
 public:
  ...
  explicit SparseArray(ulong size = 0) : BaseArray<T>(false) { _size = size; }
  ...
  //! array \warning : The ownership of the data is given to the returned
...
// Constructor
template <typename T>
SparseArray<T>::SparseArray(ulong size, ulong size_sparse, INDICE_TYPE *indices,
                            T *data)
    : BaseArray<T>(false) {
#ifdef DEBUG_ARRAY
  std::cout << "SparseArray Constructor : SparseArray(size=" << _size
            << ", size_sparse=" << size_sparse << ", indices=" << indices
...
//     - If the BaseArray is an Array, then the created array is a view (so it
...
tick::TemporaryLog<E> &operator<<(tick::TemporaryLog<E> &log,
                                  const SparseArray<T> &arr) {
  const auto size = arr.size();
  const auto size_sparse = arr.size_sparse();

  log << "SparseArray[size=" << size << ", size_sparse=" << size_sparse << ", "
      << typeid(T).name();

  if (size_sparse <= 20) {
    for (ulong i = 0; i < size_sparse; ++i)
      log << ", " << arr.indices()[i] << "/" << arr.data()[i];
  } else {
