#ifndef LIB_INCLUDE_TICK_ARRAY_SSPARSEARRAY2D_H_
#define LIB_INCLUDE_TICK_ARRAY_SSPARSEARRAY2D_H_
...
class SSparseArray2d : public SparseArray2d<T> {
 ...
 public:
#ifdef PYTHON_LINK
  void give_data_indices_rowindices_owners(void *data_owner,
                                           void *indices_owner,
                                           void *rowindices_owner);
  ...
  //! to the indices array \param row_indices A pointer to the row_indices array
  //! \param n_rows The number of rows of the full array
  //! \param n_cols The number of columns of the full array
  //! \param data_owner A pointer to the numpy array (if nullptr, then the data
  //! allocation is
  //!         owned by the object itself) that owns _data allocation
  //! \param indices_owner A pointer to the numpy array (if nullptr, then the
  //! data allocation is
  //!         owned by the object itself) that owns _indices allocation
  //! \param row_indices_owner A pointer to the numpy array (if nullptr, then
  //! the data allocation
  ...
  virtual void set_data_indices_rowindices(T *data, INDICE_TYPE *indices,
                                           ...
                                           ulong n_rows, ulong n_cols,
                                           void *owner_data = nullptr,
                                           void *owner_indices = nullptr,
                                           void *owner_row_indices = nullptr);
#else
  //! @brief Sets the data. After this call, allocation is owned by the
  //! SArray2d. \param data A pointer to the data array \param indices A pointer
  //! to the indices array \param row_indices A pointer to the row_indices array
  //! \param n_rows The number of rows of the full array
  //! \param n_cols The number of columns of the full array
  virtual void set_data_indices_rowindices(T *data, INDICE_TYPE *indices,
 ...
 public:
  ...
  // Typically after this call one should call the set_data_indices_rowindices
  // method.s
...
void SSparseArray2d<T>::set_data_indices_rowindices(
  ...
  give_data_indices_rowindices_owners(data_owner, indices_owner,
                                      row_indices_owner);
}
#else
template <typename T>
...
//        THROWSTR("WEIRED give_data_indices_rowindices_owners");
  ...
  if (_data_owner == nullptr)
    std::cout << "SSparseArray2d : SetOwner data_owner=" << data_owner << " on "
              << this << std::endl;
  else
    std::cout << "SSparseArray2d : ChangeDataOwner data_owner=" << _data_owner
              << " -> " << data_owner << " on " << this << std::endl;
  if (_indices_owner == nullptr)
    std::cout << "SSparseArray2d : SetOwner indices_owner=" << data_owner
              << " on " << this << std::endl;
  else
    std::cout << "SSparseArray2d : ChangeIndicesOwner indices_owner="
              << _indices_owner << " -> " << indices_owner << " on " << this
              << std::endl;
  if (_row_indices_owner == nullptr)
    std::cout << "SSparseArray2d : SetOwner row_indices_owner="
              << row_indices_owner << " on " << this << std::endl;
  else
    std::cout << "SSparseArray2d : ChangeRowIndicesOwner row_indices_owner="
              << _row_indices_owner << " -> " << row_indices_owner << " on "
              << this << std::endl;
#endif
...
#ifdef DEBUG_SHAREDARRAY
  n_allocs++;
  std::cout << "SSparseArray2d Constructor (->#" << n_allocs
            << ") : SSparseArray2d(n_rows=" << _n_rows << ",n_cols=" << _n_cols
            << ") --> " << this << std::endl;
#endif
}
...
std::shared_ptr<SSparseArray2d<T>> SSparseArray2d<T>::new_ptr(
  ...
  memcpy(aptr->row_indices(), a.row_indices(),
         sizeof(INDICE_TYPE) * (a.n_rows() + 1));
  return aptr;
}
...
void SSparseArray2d<T>::_clear(bool &flag_desallocate_data,
  ...
  flag_desallocate_data = flag_desallocate_indices =
      flag_desallocate_row_indices = false;
...
#ifdef DEBUG_SHAREDARRAY
      std::cout << "SSparseArray2d Clear : " << this
                << " decided not to free row_indices=" << _row_indices
                << " since owned by owner=" << _row_indices_owner << std::endl;
#endif
      _row_indices_owner = nullptr;
    } else {
#ifdef DEBUG_SHAREDARRAY
      std::cout << "SSparseArray2d Clear : " << this
                << " decided to free data=" << _row_indices_owner << std::endl;
#endif
...
#ifdef DEBUG_SHAREDARRAY
    std::cout << "SSparseArray2d Clear : " << this
              << " decided to free row_indices=" << _row_indices << std::endl;
#endif
    flag_desallocate_row_indices = true;
#endif
  }
...
SSparseArray2d<T>::~SSparseArray2d() {
  ...
  std::cout << "SSparseArray2d Destructor (->#" << n_allocs
            << ") : ~SSparseArray2d on " << this << std::endl;
#endif
  clear();
}

// @brief Returns a shared pointer to a SSparseArray2d encapsulating the sparse
