...
#ifdef DEBUG_SHAREDARRAY
    Str << "SArray2d<" << typeid(T).name() << ">(" << reinterpret_cast<void *>(v) << ", n_rows=" << v->n_rows() << ",n_cols=" << v->n_cols() << ")";
#else
    Str << "SArray2d<" << typeid(T).name() << ">(" << reinterpret_cast<void *>(v) << ",n_rows=" << v->n_rows() << ",n_cols=" << v->n_cols() << ")";
#endif
    return Str;
...
#ifdef DEBUG_SHAREDARRAY
    n_allocs++;
    std::cout << "SArray2d<" << typeid(T).name() << "> Constructor (->#" << n_allocs << ") : SArray2d(n_rows=" << _n_rows
...
#ifdef DEBUG_SHAREDARRAY
    n_allocs--;
    std::cout << "SArray2d<" << typeid(T).name() << "> Destructor (->#" << n_allocs << ") : ~SArray2d on " <<  this << std::endl;
#endif
    clear();
}
