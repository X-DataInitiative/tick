...
struct vector_operations_cblas<float> final
...
#if (defined(__APPLE__))
  void solve_linear_system(size_t n, float *A, float *b, int* ipiv=nullptr) const {
    int n_cols = 1;
    int lda = n;
  ...
  void solve_positive_symmetric_linear_system(size_t n, float *A, float *b, int* ipiv=nullptr,
                                     int switch_linear=30) const {
...
struct vector_operations_cblas<double> final
  ...
  void solve_linear_system(size_t n, double *A, double *b, int *ipiv = nullptr) const {
    int n_cols = 1;
    int lda = n;
    bool should_free = false;
    if (ipiv == nullptr) {
  ...
  void solve_positive_symmetric_linear_system(size_t n, double *A, double *b, int *ipiv = nullptr,
                                              int switch_linear = 30) const {
    // it seems faster this way with BLAS
    if (n < switch_linear) return solve_linear_system(n, A, b, ipiv);
    int n_cols = 1;
