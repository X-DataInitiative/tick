 ...
 * decay)
 */
class DLL_PUBLIC ModelHawkesCustom : public ModelHawkesLogLikSingle {
 protected:
 ...
 public:
  ...
  ModelHawkesCustom() : ModelHawkesLogLikSingle(0), decay(0) {}
 ...
 private:
   ...
   */
  ulong get_alpha_i_first_index(const ulong i) const { return n_nodes + i * n_nodes; }

  /**
   * @brief Return the end of alpha i coefficients in a coeffs vector
   * @param i : selected dimension
   */
  ulong get_alpha_i_last_index(const ulong i) const { return n_nodes + (i + 1) * n_nodes; }
 ...
 public:
  ulong get_n_coeffs() const { return n_nodes + n_nodes * n_nodes + n_nodes * (MaxN_of_f - 1); }
