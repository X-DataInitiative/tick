...
class DLL_PUBLIC ModelHawkesFixedExpKernCustomLogLikList : public ModelHawkesCustomLogLikList {
 ...
 public:
  ModelHawkesFixedExpKernCustomLogLikList(const double &_decay, const ulong _MaxN_of_f,
  ...
  std::unique_ptr<ModelHawkesLogLikSingle> build_model(const int n_threads) {
    return std::unique_ptr<ModelHawkesCustom>(new ModelHawkesCustom(decay, MaxN_of_f, n_threads));
  }
