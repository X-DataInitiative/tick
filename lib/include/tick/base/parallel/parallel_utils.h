...
/**
 * Determines the return value resulting from calling method type 'T' on object
 ...
 *
 * @return True if return type of function call is a Python primitive, otherwise
 * false
 */
template <typename T, typename S, typename... Args>
 ...
 *
 * @param exceptions Collection of possible exceptions (represented by set of
 * std::exception_ptr)
 */
 ...
 *
 * It is specialized in two cases, one for return types we can return to Python
 * (integers and floating points) and one for all other types.
 *
 * Each specialization defines 'type' which is the returned collection, and 'RT'
 * which is the value type of the returned collection.
 */
