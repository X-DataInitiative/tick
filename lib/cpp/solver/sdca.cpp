...
template <class T, class K>
void TBaseSDCA<T, K>::solve(size_t n_epochs) {
  prepare_solve();
...
template <class T, class K>
void TBaseSDCA<T, K>::solve_batch(size_t n_epochs, ulong batch_size) {
  prepare_solve();
...
template <class T>
TSDCA<T>::TSDCA(T l_l2sq, ulong epoch_size, T tol, RandType rand_type, size_t record_every, int seed, int n_threads)
