...
) : BaseSAGA<double, double>(epoch_size, tol, rand_type, step, seed, variance_reduction)
{}

template <class T, class K>
void
BaseSAGA<T, K>::set_model(std::shared_ptr<TModel<T, K> > model) {
  // model must be a child of ModelGeneralizedLinear
  auto casted_model = std::dynamic_pointer_cast<TModelGeneralizedLinear<T, K> >(model);
...
template <class T, class K>
void
BaseSAGA<T, K>::initialize_solver() {
...
BaseSAGA<T, K>::prepare_solve() {
  ...
  rand_index = 0;
  if (variance_reduction == BaseSAGA::VarianceReductionMethod::Average) {
    next_iterate.init_to_zero();
  }
  if (variance_reduction == BaseSAGA::VarianceReductionMethod::Random) {
...
template <class T, class K>
void
BaseSAGA<T, K>::compute_step_corrections() {
  ulong n_features = model->get_n_features();
  std::shared_ptr<TModelLabelsFeatures<T, K> > casted_model;
  casted_model = std::dynamic_pointer_cast<TModelLabelsFeatures<T, K> >(model);
  Array<K> columns_sparsity = casted_model->get_column_sparsity_view();
  steps_correction = Array<K>(n_features);
...
template <class T, class K>
void
BaseSAGA<T, K>::solve_dense(bool use_intercept, ulong n_features) {
  ...
  for (ulong t = 0; t < epoch_size; ++t) {
    ...
    BaseArray<K> x_i = model->get_features(i);
    ...
    for (ulong j = 0; j < n_features; ++j) {
      ...
      decrement_iterate_j_dense(j, step, x_ij, grad_factor_diff, grad_avg_j);
    ...
    if (variance_reduction == BaseSAGA::VarianceReductionMethod::Random && t == rand_index) {
      next_iterate = iterate;
    }
    if (variance_reduction == BaseSAGA::VarianceReductionMethod::Average) {
  ...
  if (variance_reduction == BaseSAGA::VarianceReductionMethod::Last) {
    next_iterate = iterate;
  }
...
template <class T, class K>
void
BaseSAGA<T, K>::solve_sparse_proba_updates(bool use_intercept, ulong n_features) {
  ...
  for (ulong t = 0; t < epoch_size; ++t) {
    ...
    for (ulong idx_nnz = 0; idx_nnz < x_i.size_sparse(); ++idx_nnz) {
      ...
      decrement_iterate_j_sparse(j, step, x_ij, grad_factor_diff, grad_avg_j);
      gradients_average[j] += grad_factor_diff * x_ij / n_samples;
...
template <class T, class K>
void
BaseSAGA<T, K>::set_starting_iterate(Array<T> &new_iterate) {
  TStoSolver<T, K>::set_starting_iterate(new_iterate);
  next_iterate = iterate;
}

template class BaseSAGA<double, double>;
template class BaseSAGA<float , float>;
