...
#include "tick/hawkes/model/list_of_realizations/hawkes_fixed_custom_loglik_list.h"
...
void ModelHawkesCustomLogLikList::compute_weights() {
  if (!model_list.empty() && timestamps_list.size() != model_list.size()) {
    TICK_ERROR(
        "Cannot compute weights as timestamps have not been stored. "
  ...
  parallel_run(get_n_threads(), n_realizations * n_nodes,
               &ModelHawkesCustomLogLikList::compute_weights_i_r, this);
...
//////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
void ModelHawkesCustomLogLikList::incremental_set_data(
...
std::tuple<ulong, ulong> ModelHawkesCustomLogLikList::get_realization_node(ulong i_r) {
    const ulong r = static_cast<const ulong>(i_r / n_nodes);
    const ulong i = i_r % n_nodes;
    return std::make_tuple(r, i);
}

void ModelHawkesCustomLogLikList::compute_weights_i_r(const ulong i_r) {
    ulong r, i;
...
double ModelHawkesCustomLogLikList::loss(const ArrayDouble &coeffs) {
...
void ModelHawkesCustomLogLikList::grad_i_r(const ulong i_r,
                                           ArrayDouble &out,
...
void ModelHawkesCustomLogLikList::grad(const ArrayDouble &coeffs, ArrayDouble &out) {
    ...
    parallel_map_array<ArrayDouble>(
            ...
            &ModelHawkesCustomLogLikList::grad_i_r,
            this, out, coeffs);
    out /= Total_events;
}

void ModelHawkesCustomLogLikList::grad_i(const ulong i, const ArrayDouble &coeffs,
                                         ArrayDouble &out) {
    if (!weights_computed) compute_weights();
    const auto r_i = sampled_i_to_realization(i);
...
double ModelHawkesCustomLogLikList::loss_and_grad(const ArrayDouble &coeffs,
                                                  ArrayDouble &out) {
...
std::pair<ulong, ulong> ModelHawkesCustomLogLikList::sampled_i_to_realization(
