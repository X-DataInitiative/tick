...
ModelHawkesExpKernLeastSq::ModelHawkesExpKernLeastSq(
    ...
    : ModelHawkesLeastSq(max_n_threads, optimization_level), decays(decays) {
  aggregated_model = std::unique_ptr<ModelHawkesExpKernLeastSqSingle>(
      new ModelHawkesExpKernLeastSqSingle(decays, max_n_threads,
                                          optimization_level));
}
...
void ModelHawkesExpKernLeastSq::hessian(ArrayDouble &out) {
  ...
  auto *casted_model =
      static_cast<ModelHawkesExpKernLeastSqSingle *>(aggregated_model.get());
...
void ModelHawkesExpKernLeastSq::compute_weights_timestamps_list() {
  ...
  for (ulong r = 0; r < n_realizations; ++r) {
    model_list[r] =
        ModelHawkesExpKernLeastSqSingle(decays, 1, optimization_level);
  ...
  parallel_run(get_n_threads(), n_realizations * n_nodes,
               &ModelHawkesExpKernLeastSq::compute_weights_i_r, this,
               model_list);
...
void ModelHawkesExpKernLeastSq::compute_weights_timestamps(
  ...
  auto model = ModelHawkesExpKernLeastSqSingle(decays, get_n_threads(),
                                               optimization_level);
...
void ModelHawkesExpKernLeastSq::compute_penalization_constant(double x,
                                                              ArrayDouble &pen_mu,
