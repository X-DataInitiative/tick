...
ModelHawkesCustom::ModelHawkesCustom(const double _decay, const ulong _MaxN_of_f,
...
void ModelHawkesCustom::allocate_weights() {
  ...
  for (ulong i = 0; i != n_nodes; i++) {
    // 0 + events + T
    g[i] = ArrayDouble2d(n_total_jumps + 2, n_nodes);
    g[i].init_to_zero();
    G[i] = ArrayDouble2d(n_total_jumps + 2, n_nodes);
    G[i].init_to_zero();
  ...
  for (ulong j = 0; j != n_nodes; j++) {
    ...
    for (ulong k = 1; k != 1 + n_total_jumps + 1; k++) {
      ...
      if (k != 1 + n_total_jumps)
        g_i[k * n_nodes + j] =
            g_i[(k - 1) * n_nodes + j] * ebt + (type_n[k - 1] == j + 1 ? decay * ebt : 0);
      G_i[k * n_nodes + j] =
...
double ModelHawkesCustom::loss(const ArrayDouble &coeffs) {
  ...
  const double loss = parallel_map_additive_reduce(get_n_threads(), n_nodes,
                                                   &ModelHawkesCustom::loss_dim_i, this, coeffs);
  return loss / Total_events;
}

double ModelHawkesCustom::loss_dim_i(const ulong i, const ArrayDouble &coeffs) {
    ...
    if (type_n[k] == i + 1) {
      ...
      if (tmp_s <= 0) {
        TICK_ERROR(
            ...
            "your proximal operator, in ModelHawkesCustom::loss_dim_i");
      }
      loss += log(tmp_s);
    }
...
void ModelHawkesCustom::grad(const ArrayDouble &coeffs, ArrayDouble &out) {
  if (!weights_computed) compute_weights();
  out.fill(0);

  // This allows to run in a multithreaded environment the computation of each component
  parallel_run(get_n_threads(), n_nodes, &ModelHawkesCustom::grad_dim_i, this, coeffs, out);
  out /= Total_events;

  for (ulong k = 0; k != get_n_coeffs(); ++k) out[k] = -out[k];
}

void ModelHawkesCustom::grad_dim_i(const ulong i, const ArrayDouble &coeffs, ArrayDouble &out) {
  const double mu_i = coeffs[i];
