...
#include "tick/hawkes/model/list_of_realizations/hawkes_fixed_sumexpkern_loglik_custom2.h"

ModelHawkesSumExpCustomType2::ModelHawkesSumExpCustomType2(const ArrayDouble &_decays,
                                                           const ulong _MaxN,
                                                           const int max_n_threads)
    : ModelHawkesLogLikSingle(max_n_threads), MaxN(_MaxN), decays(_decays) {}
...
void ModelHawkesSumExpCustomType2::compute_weights_dim_i(const ulong i) {
  ...
  //! hacked code here, seperator = 1
  //  meaning L(increasing) is timestamps[1], C,M(decreasing) are timestamps[2] timestamps[3]
...
double ModelHawkesSumExpCustomType2::loss(const ArrayDouble &coeffs) {
  ...
  const double loss = parallel_map_additive_reduce(
      get_n_threads(), n_nodes, &ModelHawkesSumExpCustomType2::loss_dim_i, this, coeffs);
  return loss / Total_events;
}

double ModelHawkesSumExpCustomType2::loss_dim_i(const ulong i, const ArrayDouble &coeffs) {
  double loss = 0;
...
void ModelHawkesSumExpCustomType2::grad(const ArrayDouble &coeffs, ArrayDouble &out) {
  if (!weights_computed) compute_weights();
  out.fill(0);

  // This allows to run in a multithreaded environment the computation of each component
  parallel_run(get_n_threads(), n_nodes, &ModelHawkesSumExpCustomType2::grad_dim_i, this, coeffs,
               out);
  out /= Total_events;
...
void ModelHawkesSumExpCustomType2::grad_dim_i(const ulong i, const ArrayDouble &coeffs,
                                              ArrayDouble &out) {
  const ArrayDouble mu_i = view(coeffs, get_mu_i_first_index(i), get_mu_i_last_index(i));
