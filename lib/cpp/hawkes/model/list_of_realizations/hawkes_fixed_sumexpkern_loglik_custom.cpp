...
#include "tick/hawkes/model/list_of_realizations/hawkes_fixed_sumexpkern_loglik_custom.h"

ModelHawkesSumExpCustom::ModelHawkesSumExpCustom(const ArrayDouble _decays, const ulong _MaxN_of_f,
                                                 const int max_n_threads)
    : ModelHawkesLogLikSingle(max_n_threads), MaxN_of_f(_MaxN_of_f), decays(_decays) {}
...
void ModelHawkesSumExpCustom::compute_weights_dim_i(const ulong i) {
  ...
  //! hacked code here, seperator = 1, meaning L(increasing) is timestamps[1], C,M(decreasing) are
...
double ModelHawkesSumExpCustom::loss(const ArrayDouble &coeffs) {
  ...
  const double loss = parallel_map_additive_reduce(
      get_n_threads(), n_nodes, &ModelHawkesSumExpCustom::loss_dim_i, this, coeffs);
  return loss / Total_events;
}

double ModelHawkesSumExpCustom::loss_dim_i(const ulong i, const ArrayDouble &coeffs) {
...
void ModelHawkesSumExpCustom::grad_dim_i(const ulong i, const ArrayDouble &coeffs,
                                         ArrayDouble &out) {
  const double mu_i = coeffs[i];
...
void ModelHawkesSumExpCustom::grad(const ArrayDouble &coeffs, ArrayDouble &out) {
  if (!weights_computed) compute_weights();
  out.fill(0);

  // This allows to run in a multithreaded environment the computation of each component
  parallel_run(get_n_threads(), n_nodes, &ModelHawkesSumExpCustom::grad_dim_i, this, coeffs, out);
  out /= Total_events;
