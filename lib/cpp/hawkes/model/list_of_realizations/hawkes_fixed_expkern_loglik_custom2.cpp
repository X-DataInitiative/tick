...
#include "tick/hawkes/model/list_of_realizations/hawkes_fixed_expkern_loglik_custom2.h"

ModelHawkesCustomType2::ModelHawkesCustomType2(const double _decay, const ulong _MaxN,
                                               const int max_n_threads)
    : ModelHawkesLogLikSingle(max_n_threads), MaxN(_MaxN), decay(_decay) {}
...
double ModelHawkesCustomType2::loss(const ArrayDouble &coeffs) {
  if (!weights_computed) compute_weights();

  const double loss = parallel_map_additive_reduce(
      get_n_threads(), n_nodes, &ModelHawkesCustomType2::loss_dim_i, this, coeffs);
  return loss / Total_events;
}

double ModelHawkesCustomType2::loss_dim_i(const ulong i, const ArrayDouble &coeffs) {
  double loss = 0;
...
void ModelHawkesCustomType2::grad(const ArrayDouble &coeffs, ArrayDouble &out) {
  if (!weights_computed) compute_weights();
  out.fill(0);

  // This allows to run in a multithreaded environment the computation of each component
  parallel_run(get_n_threads(), n_nodes, &ModelHawkesCustomType2::grad_dim_i, this, coeffs, out);
  out /= Total_events;

  for (ulong k = 0; k != get_n_coeffs(); ++k) out[k] = -out[k];
}

void ModelHawkesCustomType2::grad_dim_i(const ulong i, const ArrayDouble &coeffs,
                                        ArrayDouble &out) {
  const ArrayDouble mu_i = view(coeffs, get_mu_i_first_index(i), get_mu_i_last_index(i));
