...
ModelCustomBasic::ModelCustomBasic(const double _decay, const ulong _MaxN_of_f,
                                   const int max_n_threads)
    : ModelHawkesLogLikSingle(max_n_threads), MaxN_of_f(_MaxN_of_f), decay(_decay) {}
...
void ModelCustomBasic::compute_weights_dim_i(const ulong i) {
  ...
  for (ulong k = 1; k != 1 + n_total_jumps + 1; k++) {
    if (k != (1 + n_total_jumps)) H1_i[global_n[k - 1]] += (type_n[k] == i + 1 ? 1 : 0);
...
void ModelCustomBasic::set_data(const SArrayDoublePtrList1D &_timestamps,
                                const SArrayLongPtr _global_n, const double _end_times) {
  // copied from single - refactor to single static/templated later
  ...
  for (ulong k = 0; k != n_total_jumps + 1; ++k) global_n[k] = _global_n->value(k);
  ...
  for (ulong k = 1; k != n_total_jumps + 1; ++k) type_n[k] = tmp_pre_type_n[tmp_index[k]];
...
double ModelCustomBasic::loss(const ArrayDouble &coeffs) {
  ...
  const double loss = parallel_map_additive_reduce(get_n_threads(), n_nodes,
                                                   &ModelCustomBasic::loss_dim_i, this, coeffs);
  return loss / Total_events;
}

double ModelCustomBasic::loss_dim_i(const ulong i, const ArrayDouble &coeffs) {
  ...
  f_i[0] = 1;
  for (ulong k = 1; k != MaxN_of_f; ++k) f_i[k] = coeffs[n_nodes + i * (MaxN_of_f - 1) + k - 1];
  ...
  for (ulong k = 1; k != n_total_jumps + 1; k++)
    //! insert event t0 = 0
    if (type_n[k] == i + 1) loss += log(f_i[global_n[k - 1]]);
...
void ModelCustomBasic::grad(const ArrayDouble &coeffs, ArrayDouble &out) {
  if (!weights_computed) compute_weights();
  out.fill(0);

  // This allows to run in a multithreaded environment the computation of each component
  parallel_run(get_n_threads(), n_nodes, &ModelCustomBasic::grad_dim_i, this, coeffs, out);
  out /= Total_events;

  for (ulong k = 0; k != get_n_coeffs(); ++k) out[k] = -out[k];
}

void ModelCustomBasic::grad_dim_i(const ulong i, const ArrayDouble &coeffs, ArrayDouble &out) {
  const double mu_i = coeffs[i];
  ...
  for (ulong k = 1; k != MaxN_of_f; ++k) out[n_nodes + i * (MaxN_of_f - 1) + k - 1] = grad_f_i[k];
}