...
#include "tick/hawkes/simulation/hawkes_custom.h"

Hawkes_custom::Hawkes_custom(unsigned int n_nodes, int seed, ulong _MaxN_of_f,
  ...
  for (ulong i = 0; i != n_nodes; ++i) f_i_Max[i] = f_i[i]->max();

  last_global_n = 0;
}

Hawkes_custom::Hawkes_custom(unsigned int n_nodes, int seed, ulong _MaxN_of_f,
                             const SArrayDoublePtrList1D &_f_i, const ArrayDouble &extrainfo,
                             const std::string _simu_mode)
  ...
  if (simu_mode == "random")
    ...
    for (ulong k = 0; k != dim; ++k) avg_order_size[k] = extrainfo[3 + k];
...
bool Hawkes_custom::update_time_shift_(double delay, ArrayDouble &intensity,
