...
HawkesTimeFunctionBaseline::HawkesTimeFunctionBaseline(ArrayDouble &times, ArrayDouble &values) {
  time_function = TimeFunction(times, values, TimeFunction::BorderType::Cyclic,
                               TimeFunction::InterMode::InterConstRight);
}

double HawkesTimeFunctionBaseline::get_value(double t) { return time_function.value(t); }

SArrayDoublePtr HawkesTimeFunctionBaseline::get_value(ArrayDouble &t) {
  return time_function.value(t);
}

double HawkesTimeFunctionBaseline::get_primitive_value(double t) {
  return time_function.primitive(t);
}
