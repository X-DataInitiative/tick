...
class Test(unittest.TestCase):
    def setUp(self):
        ...
        self.features = np.array([[0.00902084, 0.54159776, 0., 'z'],
    ...
    def test_column_type_detection(self):
        ...
        """
        expected_column_types = ["continuous", "continuous", "discrete",
                                 "discrete"]
        ...
        for i, expected_type in enumerate(expected_column_types):
            ...
            detected_type = FeaturesBinarizer._detect_feature_type(
                features_i, detect_column_type="auto", continuous_threshold="auto"
            )
    ...
    def test_quantile_boundaries_detection(self):
        ...
        boundaries_0 = binarizer._get_boundaries(
            self.columns[0], self.features[:, 0], fit=True)
        np.testing.assert_array_almost_equal(boundaries_0,
            np.array([-np.inf, 0.009021, 0.271109, 0.473155, np.inf]))

        boundaries_1 = binarizer._get_boundaries(
            self.columns[1], self.features[:, 1], fit=True)
        np.testing.assert_array_almost_equal(boundaries_1,
