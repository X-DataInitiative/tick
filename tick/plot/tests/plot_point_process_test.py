...
class Test(unittest.TestCase):
    ...
    def test_extract_process_interval(self):
        plot_nodes = range(2)
        end_time = 12.
        original_timestamps = [np.linspace(1., 6., 11), np.linspace(4., 10., 5)]
        original_intensity_times = np.linspace(0, end_time, 13)
        original_intensities = [np.linspace(1., 2.2, 13),
        ...
        self.assert_array_list_equal(
            timestamps,
            [np.array([5., 5.5, 6.]), np.array([5.5, 7., 8.5, 10.])])
        ...
        self.assert_array_list_equal(
            timestamps,
            [np.array([5., 5.5, 6.]), np.array([5.5, 7., 8.5])])
