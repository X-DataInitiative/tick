 ...
 * \brief Class for computing loglikelihood function and gradient for Hawkes processes with Methode Rosenbaum
 */
class DLL_PUBLIC ModelCustomBasic : public ModelHawkesFixedKernLogLik {
public:
...
public:
    ...
    ModelCustomBasic() : ModelHawkesFixedKernLogLik(0), decay(0) {}
...
private:
    ...
    ulong get_f_i_last_index(const ulong i) const {
        return n_nodes + (i + 1) * MaxN_of_f;
    }

public:
    ulong get_n_coeffs() const override{
        return n_nodes + n_nodes * (MaxN_of_f - 1);
    }
