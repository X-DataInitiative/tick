...
ModelHawkesFixedSumExpKernCustom::ModelHawkesFixedSumExpKernCustom(const ulong _MaxN_of_f, const int max_n_threads) :
...
double ModelHawkesFixedSumExpKernCustom::loss_i(const ulong sampled_i,
...
void ModelHawkesFixedSumExpKernCustom::grad_i(const ulong sampled_i,
                                        const ArrayDouble &coeffs,
                                        ArrayDouble &out) {
    if (!weights_computed) compute_weights();
...
double ModelHawkesFixedSumExpKernCustom::loss_and_grad(const ArrayDouble &coeffs,
                                                 ArrayDouble &out) {
    if (!weights_computed) compute_weights();
    out.fill(0);
    ...
    const double loss =
            parallel_map_additive_reduce(get_n_threads(), n_nodes,
                                         &ModelHawkesFixedSumExpKernCustom::loss_and_grad_dim_i,
                                         this,
                                         coeffs, out);
    out /= n_total_jumps;
    return loss / n_total_jumps;
...
double ModelHawkesFixedSumExpKernCustom::hessian_norm(const ArrayDouble &coeffs,
                                                const ArrayDouble &vector) {
    if (!weights_computed) compute_weights();

    const double norm_sum =
            parallel_map_additive_reduce(get_n_threads(), n_nodes,
                                         &ModelHawkesFixedSumExpKernCustom::hessian_norm_dim_i,
                                         this,
                                         coeffs, vector);
...
void ModelHawkesFixedSumExpKernCustom::hessian(const ArrayDouble &coeffs, ArrayDouble &out) {
    if (!weights_computed) compute_weights();

    // This allows to run in a multithreaded environment the computation of each component
    parallel_run(get_n_threads(), n_nodes, &ModelHawkesFixedSumExpKernCustom::hessian_i,
                 this, coeffs, out);
    out /= n_total_jumps;
}
...
void ModelHawkesFixedSumExpKernCustom::sampled_i_to_index(const ulong sampled_i,
                                                    ulong *i,
                                                    ulong *k) {
    ulong cum_N_i = 0;
...
double ModelHawkesFixedSumExpKernCustom::loss_i_k(const ulong i,
                                            const ulong k,
                                            const ArrayDouble &coeffs) {
    const double mu_i = coeffs[i];
    const ArrayDouble alpha_i = view(coeffs, get_alpha_i_first_index(i), get_alpha_i_last_index(i));
...
void ModelHawkesFixedSumExpKernCustom::grad_i_k(const ulong i, const ulong k,
                                          const ArrayDouble &coeffs,
                                          ArrayDouble &out) {
    const double mu_i = coeffs[i];
    const ArrayDouble alpha_i = view(coeffs, get_alpha_i_first_index(i), get_alpha_i_last_index(i));
...
double ModelHawkesFixedSumExpKernCustom::loss_and_grad_dim_i(const ulong i,
                                                       const ArrayDouble &coeffs,
                                                       ArrayDouble &out) {
    const double mu_i = coeffs[i];
    const ArrayDouble alpha_i = view(coeffs, get_alpha_i_first_index(i), get_alpha_i_last_index(i));
...
double ModelHawkesFixedSumExpKernCustom::hessian_norm_dim_i(const ulong i,
                                                      const ArrayDouble &coeffs,
                                                      const ArrayDouble &vector) {
    const double mu_i = coeffs[i];
    const ArrayDouble alpha_i = view(coeffs, get_alpha_i_first_index(i), get_alpha_i_last_index(i));
...
void ModelHawkesFixedSumExpKernCustom::hessian_i(const ulong i,
                                           const ArrayDouble &coeffs,
                                           ArrayDouble &out) {
