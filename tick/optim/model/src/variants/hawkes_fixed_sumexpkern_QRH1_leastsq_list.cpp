...
#include "hawkes_fixed_sumexpkern_QRH1_leastsq_list.h"
#include "hawkes_utils.h"

ModelHawkesFixedSumExpKernLeastSqQRH1List::ModelHawkesFixedSumExpKernLeastSqQRH1List(
...
ModelHawkesFixedSumExpKernLeastSqQRH1List::ModelHawkesFixedSumExpKernLeastSqQRH1List(const int max_n_threads):
        ModelHawkesList(max_n_threads, 0){}

void ModelHawkesFixedSumExpKernLeastSqQRH1List::set_data(const SArrayDoublePtrList2D &timestamps_list,
...
void ModelHawkesFixedSumExpKernLeastSqQRH1List::compute_weights() {
    ...
    model_list = std::vector<std::unique_ptr<ModelHawkesFixedSumExpKernLeastSqQRH1> >(n_realizations);
...
//////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
void ModelHawkesFixedSumExpKernLeastSqQRH1List::incremental_set_data(
...
std::tuple<ulong, ulong> ModelHawkesFixedSumExpKernLeastSqQRH1List::get_realization_node(ulong i_r) {
...
double ModelHawkesFixedSumExpKernLeastSqQRH1List::loss_i_r(const ulong i_r, const ArrayDouble &coeffs) {
    ulong r, i;
    std::tie(r, i) = get_realization_node(i_r);
    return model_list[r]->loss_i(i, coeffs);
}

double ModelHawkesFixedSumExpKernLeastSqQRH1List::loss(const ArrayDouble &coeffs) {
...
void ModelHawkesFixedSumExpKernLeastSqQRH1List::grad_i_r(const ulong i_r,
                                           ArrayDouble &out,
...
void ModelHawkesFixedSumExpKernLeastSqQRH1List::grad(const ArrayDouble &coeffs, ArrayDouble &out) {
    ...
    parallel_map_array<ArrayDouble>(
            ...
            &ModelHawkesFixedSumExpKernLeastSqQRH1List::grad_i_r,
            this, out, coeffs);
    out /= Total_events;
}

void ModelHawkesFixedSumExpKernLeastSqQRH1List::grad_i(const ulong i, const ArrayDouble &coeffs,
                                         ArrayDouble &out) {
    if (!weights_computed) compute_weights();
    const auto r_i = sampled_i_to_realization(i);
...
double ModelHawkesFixedSumExpKernLeastSqQRH1List::loss_and_grad(const ArrayDouble &coeffs,
                                                  ArrayDouble &out) {
...
std::pair<ulong, ulong> ModelHawkesFixedSumExpKernLeastSqQRH1List::sampled_i_to_realization(
