...
#include "hawkes_fixed_sumexpkern_QRH2_leastsq_list.h"
#include "hawkes_utils.h"

ModelHawkesFixedSumExpKernLeastSqQRH2List::ModelHawkesFixedSumExpKernLeastSqQRH2List(
        const ArrayDouble &decays, const ulong _MaxN, const int max_n_threads) :
        ModelHawkesList(max_n_threads, 0), MaxN(_MaxN), decays(decays) {}

ModelHawkesFixedSumExpKernLeastSqQRH2List::ModelHawkesFixedSumExpKernLeastSqQRH2List(const int max_n_threads):
        ModelHawkesList(max_n_threads, 0){}

void ModelHawkesFixedSumExpKernLeastSqQRH2List::set_data(const SArrayDoublePtrList2D &timestamps_list,
...
void ModelHawkesFixedSumExpKernLeastSqQRH2List::compute_weights() {
    ...
    model_list = std::vector<std::unique_ptr<ModelHawkesFixedSumExpKernLeastSqQRH2> >(n_realizations);
    ...
    parallel_run(get_n_threads(), n_realizations * n_nodes,
                 &ModelHawkesFixedSumExpKernLeastSqQRH2List::compute_weights_i_r, this);

    parallel_run(get_n_threads(), n_realizations * n_nodes,
                 &ModelHawkesFixedSumExpKernLeastSqQRH2List::compute_weights_H_j_r, this);
...
//////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
void ModelHawkesFixedSumExpKernLeastSqQRH2List::incremental_set_data(
        const SArrayDoublePtrList1D &timestamps, double end_time) {

    TICK_ERROR("incremental_set_data is not implemented!")
//    weights_computed = false;
//    if (model_list.empty()) {
...
std::tuple<ulong, ulong> ModelHawkesFixedSumExpKernLeastSqQRH2List::get_realization_node(ulong i_r) {
    const ulong r = static_cast<const ulong>(i_r / n_nodes);
    const ulong i = i_r % n_nodes;
    return std::make_tuple(r, i);
}

void ModelHawkesFixedSumExpKernLeastSqQRH2List::compute_weights_H_j_r(const ulong j_r) {
    ulong r, j;
    std::tie(r, j) = get_realization_node(j_r);
    model_list[r]->compute_weights_H_j(j);
}

void ModelHawkesFixedSumExpKernLeastSqQRH2List::compute_weights_i_r(const ulong i_r) {
    ulong r, i;
    std::tie(r, i) = get_realization_node(i_r);
    model_list[r]->compute_weights_i(i);
}

double ModelHawkesFixedSumExpKernLeastSqQRH2List::loss_i_r(const ulong i_r, const ArrayDouble &coeffs) {
    ulong r, i;
    std::tie(r, i) = get_realization_node(i_r);
    return model_list[r]->loss_i(i, coeffs);
}

double ModelHawkesFixedSumExpKernLeastSqQRH2List::loss(const ArrayDouble &coeffs) {
    if (!weights_computed) compute_weights();
    return parallel_map_additive_reduce(
            get_n_threads(), n_realizations * n_nodes,
            &ModelHawkesFixedSumExpKernLeastSqQRH2List::loss_i_r, this, coeffs) / Total_events;
}

double ModelHawkesFixedSumExpKernLeastSqQRH2List::loss_i(const ulong i, const ArrayDouble &coeffs) {
    if (!weights_computed) compute_weights();
    const auto r_i = sampled_i_to_realization(i);
    return model_list[r_i.first]->loss_i(r_i.second, coeffs);
}

void ModelHawkesFixedSumExpKernLeastSqQRH2List::grad_i_r(const ulong i_r,
                                           ArrayDouble &out,
...
void ModelHawkesFixedSumExpKernLeastSqQRH2List::grad(const ArrayDouble &coeffs, ArrayDouble &out) {
    if (!weights_computed) compute_weights();
    out.init_to_zero();
    parallel_map_array<ArrayDouble>(
            get_n_threads(), n_realizations * n_nodes,
            [](ArrayDouble &r, const ArrayDouble &s) { r.mult_incr(s, 1.0); },
            &ModelHawkesFixedSumExpKernLeastSqQRH2List::grad_i_r,
            this, out, coeffs);
    out /= Total_events;
}

void ModelHawkesFixedSumExpKernLeastSqQRH2List::grad_i(const ulong i, const ArrayDouble &coeffs,
                                         ArrayDouble &out) {
    if (!weights_computed) compute_weights();
    const auto r_i = sampled_i_to_realization(i);
...
double ModelHawkesFixedSumExpKernLeastSqQRH2List::loss_and_grad(const ArrayDouble &coeffs,
                                                  ArrayDouble &out) {
...
std::pair<ulong, ulong> ModelHawkesFixedSumExpKernLeastSqQRH2List::sampled_i_to_realization(
...
ulong ModelHawkesFixedSumExpKernLeastSqQRH2List::get_n_coeffs() const{
