...
class SDCA(SolverFirstOrderSto):
    ...
    :class:`SDCA <tick.optim.solver.SDCA>` corresponds to the
    ...
    Internally, :class:`SDCA <tick.optim.solver.SDCA>` has dedicated code when
    ...
    * S. Shalev-Shwartz and T. Zhang, Accelerated proximal stochastic dual
    ...
    def __init__(self, l_l2sq: float, epoch_size: int = None,
        ...
        if batch_size == 1:
            batch_size_enum = BatchSize_one
        elif batch_size == 2:
            batch_size_enum = BatchSize_two
        else:
            batch_size_enum = BatchSize_many
            batch_size -= 1
    ...
    def objective(self, coeffs, loss: float = None):
        ...
        if self._proj is not None:
            coeffs = self._proj.call(coeffs)

        prox_l2_value = 0.5 * self.l_l2sq * np.linalg.norm(coeffs) ** 2
        return SolverFirstOrderSto.objective(self, coeffs, None) + prox_l2_value
