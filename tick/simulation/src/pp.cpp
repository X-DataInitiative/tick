...
void PP::simulate(double end_time, ulong n_points) {
    ...
    while (time < end_time && n_total_jumps < n_points && !flag_negative_intensity) {
        ...
        if (itr_on()) {
            while (itr_time + itr_time_step < timeOfNextJump) {
        ...
        // Do not compute intensity bound here as we want new intensities but old bound that we have used for
        // the exponential law
