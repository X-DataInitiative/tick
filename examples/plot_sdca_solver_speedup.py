...
from tick.linear_model.model_linreg import AtomicModelLinReg
...
simulator = SimuLinReg(weights, n_samples=n_samples, features=features,
                       verbose=False, intercept=intercept, dtype=float)
features, labels = simulator.simulate()

model = ModelLinReg(fit_intercept=True)
model.fit(features, labels)
atomic_model = AtomicModelLinReg(fit_intercept=True)
atomic_model.fit(features, labels)

prox = ProxZero()
atomic_prox = AtomicProxZero()
...
atomic_sdca.set_model(atomic_model, model).set_prox(atomic_prox, prox)
    ...
    for n_threads in test_n_threads:
        ...
        if n_threads == 1 and False:
            solver = SDCA(penalty_strength, tol=0, verbose=False,
                          seed=seed, max_iter=40)
            solver.set_model(model).set_prox(prox)
            solver_labels += [solver.name]
        else:
            solver = AtomicSDCA(penalty_strength, tol=0, verbose=False,
                                seed=seed, max_iter=40, n_threads=n_threads,
                                batch_size=batch_size)
            solver.set_model(atomic_model, model).set_prox(atomic_prox, prox)
