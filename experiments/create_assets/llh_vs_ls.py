...
def run_simu_and_timings(adjacency, decays, baseline, end_time):
    ...
    start_weights_ls = time.time()
    model_ls._model.compute_weights()
    time_weights_ls = time.time() - start_weights_ls

    start_weights_llh = time.time()
    model_llh._model.compute_weights()
    time_weights_llh = time.time() - start_weights_llh

    start_loss_ls = time.time()
    model_ls.loss(np.ones(model_ls.n_coeffs))
    time_loss_ls = time.time() - start_loss_ls

    start_loss_llh = time.time()
    model_llh.loss(np.ones(model_ls.n_coeffs))
    time_loss_llh = time.time() - start_loss_llh
...
if __name__ == '__main__':
    ...
    csv_path_ = 'timings_llh_vs_ls.csv'
